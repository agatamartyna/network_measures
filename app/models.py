...
cont_obstr = [
    'w', 'f', 'z', 's', 'sz', 'ż', 'rz', 'ś', 'si', 'ź', 'zi', 'h', 'ch'
]
...
# non-palatals
non_pals = [
    VS_BILAB_STOP, VD_BILAB_STOP, VS_DENT_STOP, VD_DENT_STOP,
    VS_LABDENT_FRIC, VD_LABDENT_FRIC,
    '\u0261', 'x', 'k', 'l', 'm', 'r'
]
...
def transcribe(word):
    ...
    for i in range(len(ph_word) - 2):
        if ph_word[i:i + 2] == 'ɛn' or ph_word[i:i + 2] == 'ɔn':
            ...
            elif ph_word[i + 2:i + 4] in [VS_ALV_AFF, VD_ALV_AFF,
                                          VS_POSTALV_AFF, VD_POSTALV_AFF]:
    ...
    # this empirically-based solution not working
    # with final obstruent clusters longer than 2 items
